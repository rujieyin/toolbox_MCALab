...
% of the image in each dictionary.
	deltamax=StartingPoint(coeff,dict,n);
...
for nb=1:nbdicts
 ...
 if strcmp(nameofdict,'CURVWRAP')
     coeffs(1).coeff{1} = zeros(size(coeffs(1).coeff{1}));
     for j = 2:scaleindex
     ...
     for j = 2:scaleindex
         if strcmp(nameofdict,'CDWT2')
             coeffs(j).coeff = coeffs(j).coeff.*(abs(coeffs(j).coeff) > lambda*.5);
         else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function delta = StartingPoint(C,dict,nn)
global E
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function computeL2norm(coeffs,nn)
...
C = fdct_wrapping(X,1,1,length(coeffs)); 		% Get the curvelets
